...
def _download_from_key(
    ...
    for f in reduced_files:
        if not os.path.isfile(os.path.join(data_path, "processed", f.split("/")[-1])):
            ...
            download(f, os.path.join(localpath, f.split("/")[-1]))  # Just the file name in the list of objects
...
def download_clean_from_s3(
    ...
    if not file:  # No single file passed, so download recursively
        ...
        download(os.path.join("jlehrer", "expression_data", "processed", file), local_path)
...
def download_raw_from_s3(
    ...
    if not file:
        ...
        download(os.path.join("jlehrer", "expression_data", "raw", file), local_path)
...
def download_raw_expression_matrices(
    datasets: Dict[str, Tuple[str, str]], upload: bool = False, unzip: bool = True, datapath: str = None
) -> None:
    ...
    for file, links in datasets.items():
        ...
        if upload:
            ...
            upload(datafile_path, os.path.join("jlehrer", "expression_data", "raw", file))
...
class UploadCallback(pl.callbacks.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        if epoch % self.epochs == 0 and epoch > 0:  # Save every ten epochs
            ...
            upload(os.path.join(self.path, checkpoint), os.path.join("jlehrer", self.upload_path, checkpoint))
